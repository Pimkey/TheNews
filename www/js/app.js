function buildSourceHtml(element) {
    var elementHtml = '<li><a href="#">' +
        ...
        '<a href="#add_to_list" data-source-id="' + element.id + '" data-rel="popup" data-position-to="window" data-transition="pop" class="ui-btn ui-btn-icon-notext ui-icon-gear ui-btn-a">Add to list</a>' +
        '</a></li>';
    return elementHtml;
}
...
function addToListsListView(listId, object, classes) {
    if (classes != undefined && classes != null & classes.trim() != '') {
        $("#" + listId).append('<li data-list-id="' + object.id + '" class="' + classes + '">' + object.name + '</li>');
    } else {
        $("#" + listId).append('<li data-list-id="' + object.id + '"><a href="#">' + object.name + '<a></li>');
    }
}
